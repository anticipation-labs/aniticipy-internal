...
import { CANOPY_BASE_PATH, CANOPY_BASE_PATH_HEADER } from "../base-path";
 ...
 */
export function callbackUrl(reqUrl: string, basePath = ""): string {
  const u = new URL(reqUrl);
  const isLocal = u.hostname === "localhost" || u.hostname === "127.0.0.1";
  const scheme = isLocal ? u.protocol.replace(/:$/, "") : "https";
  return `${scheme}://${u.host}${basePath}/auth/callback`;
}

function requestBasePath(c: { req: { header(name: string): string | undefined } }): string {
  return c.req.header(CANOPY_BASE_PATH_HEADER) === CANOPY_BASE_PATH ? CANOPY_BASE_PATH : "";
}

function requestPath(c: { req: { header(name: string): string | undefined } }, path: string): string {
  return `${requestBasePath(c)}${path}`;
}

function cookiePath(c: { req: { header(name: string): string | undefined } }): string {
  return requestBasePath(c) || "/";
}
...
authApp.get("/login", async (c) => {
  ...
  setCookie(c, OAUTH_TX_COOKIE, sealed, { httpOnly: true, secure: true, sameSite: "Lax", path: cookiePath(c), maxAge: 600 });
  return c.redirect(
    buildAuthorizeUrl({ clientId: c.env.GITHUB_CLIENT_ID, redirectUri: callbackUrl(c.req.url, requestBasePath(c)), state, challenge }),
    302
  );
});
...
authApp.get("/callback", async (c) => {
  ...
  deleteCookie(c, OAUTH_TX_COOKIE, { path: cookiePath(c) });
  ...
  const token = await exchangeCode({ env: c.env, code, redirectUri: callbackUrl(c.req.url, requestBasePath(c)), verifier });
  ...
  if (!(await isActiveOrgMember(token, orgFor(c.env)))) return c.redirect(requestPath(c, "/?denied=1"), 302);
  ...
  await setSessionCookie(c, id, c.env.COOKIE_SECRET, cookiePath(c));
  return c.redirect(requestPath(c, "/"), 302);
});
...
authApp.post("/logout", async (c) => {
  ...
  clearSessionCookie(c, cookiePath(c));
